...
<dl class="company_center_aside">
	...
	<dd >
		<a href="positionsdown.php">已下线职位</a>
            ...
            <div class="content">
            	<dl class="company_center_content">
                    ...
                    <dd>
                   	  <form action="undeal.php" method="get" id="filterForm">
                    	<div class="filter_actions">
     <p>

     </p>
                    	  <p><a ></a>求职简历&nbsp;  &nbsp; &nbsp; &nbsp;    <select name="sex" >
     <optgroup label="请选择性别">
     <option>男</option>
     <option>女</option>
     </optgroup></select>
          <select name="record" >
     <optgroup label="请选择学历">
     <option>大专</option>
     <option>本科</option>
     <option>硕士</option>
     <option>博士</option>
     ...
     <input type="submit" value="筛选"/></p>
                   	    </a></div><!-- end .filter_actions -->
...
if(isset($_GET['sex'],$_GET['record'])){
	$sql="select orders.oid,orders.jid,orders.id,orders.cid,orders.ostate,orders.otime from orders inner join student on orders.id=student.id where student.sex='{$_GET['sex']}' and student.record='{$_GET['record']}';";
	}
	else{$sql="select * from orders where cid='{$_COOKIE['id']}' and ostate='未查阅' ";}
...
while
	...
	$zwjs=$row2[17];
		...
		$sql3="select * from job where jid='{$jid}' ORDER BY jid DESC";
